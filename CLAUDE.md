...
**📊 Status board:** ✅ 40 shipped · 🔨 1 in build · 💤 9 queued (50 tracked) — full tables in [`STATUS.md`](STATUS.md), source of truth [`brain/roadmap.json`](brain/roadmap.json).
<!-- STATUS:END -->
...
| Per-user vault (browser) | localStorage `hermes.vault.v1`, `hermes.albums.v1`, `hermes.taste.v1`, `hermes.bannedWords.v1`, `hermes.artistAlias.v1`, `hermes.favorites.v1` (each mirrored to a `.bak` key) via `lib/hermes/storage.ts` |
