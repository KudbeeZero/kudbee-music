...
| [`modelFamily.json`](modelFamily.json) | The model-family card catalog — the Librarian's ledger for the KUDBEE/HERMES training program: per-model status, dataset lineage, evals with run counts, KUDBEE-GATE promotion state, budgets, train order. | `lib/hermes/modelFamily.ts` (invariants CI-enforced by `modelFamily.test.ts`); read/written by the Lightning agent every GPU session (`docs/lightning-librarian.md`) | Live |
| [`handoffs.json`](handoffs.json) | The cross-agent handoff log — the async comms channel between the kudbee-music session and the Lightning GPU agent (read at session start, append at session end) so neither works blind. Broader than the model catalog: GPU status, blockers, review requests, founder directives. | Read/appended by both agents by hand each session (protocol inside the file); `docs/agent-autonomy.md` context | Live |
