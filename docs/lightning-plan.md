...
## Future option: orchestrating the training loop (deferred, not built)

Once this becomes a real recurring cycle — data prep → upload → trigger a Lightning
fine-tune → poll the job → run the eval rubric → compare against the current baseline
(the pass-rate number the TDE Memory/Training panel shows) → human approval → promote
the adapter — that sequence is a genuine multi-step pipeline, not a one-off script.
[Kestra](https://kestra.io/) (open-source, YAML-declarative, retries/scheduling/
webhook-triggers/human-approval-gates/audit-trail built in) is worth evaluating *at
that point* as the engine instead of hand-rolling retry and audit logic. Not needed
today — the loop is still one manual CLI call against one Studio — and it would live
entirely in founder-side ops tooling, never the client bundle (same `.env.local`
boundary as the Lightning key itself). Flagged here so it isn't reinvented from
scratch when the loop actually grows into one.
